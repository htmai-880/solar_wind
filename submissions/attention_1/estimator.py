...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sequence = self.X.iloc[start:index]
        # print(sequence)
        sequence = torch.tensor(sequence.values)
        if not self.y is None:
            label = torch.tensor([self.y[index+1]], dtype=torch.int64)
...
class Classifier(BaseEstimator):
    ...
    def predict(self, X):
        ...
        y = []
        data = data_loader = get_loader(
            X, y, max_len=self.max_len, batch_size=self.batch_size)
        for idx, data in enumerate(data_loader):
